...
fun concat23Trees(items: MutableList<BaseAstNode>): BaseAstNode? {
    ...
    fun readNode(): BaseAstNode? {
        ...
        return if (i - start >= 2) {
            val pooledSlice = TokenAllocator.obtainNodeList()
            pooledSlice.ensureCapacity(i - start)
            for (idx in start until i) {
                pooledSlice.add(items[idx])
            }
            val node = concat23TreesOfSameHeight(pooledSlice, false)
            TokenAllocator.recycleNodeList(pooledSlice)
            node
        } else {
            items[start]
        }
